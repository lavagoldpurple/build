...
[[ -f "$root_mount/etc/mysql/mariadb.conf.d/90-aibox.cnf" ]] || { echo 'MariaDB config missing' >&2; exit 1; }
grep -Fq 'datadir=/userdata/aibox/database/mysql' "$root_mount/etc/mysql/mariadb.conf.d/90-aibox.cnf" || { echo 'MariaDB datadir mismatch' >&2; exit 1; }
[[ -f "$root_mount/lib/systemd/system/aibox-storage-init.service" ]] || { echo 'storage service missing' >&2; exit 1; }
[[ -f "$root_mount/lib/systemd/system/aibox-mariadb-bootstrap.service" ]] || { echo 'MariaDB bootstrap service missing' >&2; exit 1; }
